...
For this, we will need two things: A ``WORKSPACE`` and a ``PROJECT`` to work on.
...
Now, we can go back to the root and build it using ``colcon``::

   colcon build

The last step is to source our workspace so ROS2 is aware of it::
...
ROS2 comes preinstalled with a tool for package generation::

   # In src:
   ros2 pkg create --build-type ament_python <package_name>
   # Make sure you replace <package_name> with a desired name
