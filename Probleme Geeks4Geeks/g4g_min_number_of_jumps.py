...
can be made forward from that element. This means if arr[i] = x, then we can jump any distance y such that y ≤ x.
Find the minimum number of jumps to reach the end of the array (starting from the first element). If an element 
...
class Solution:
    ...
    def minJumps3(self, arr, n): #Furata de pe g4g
	    #code here
        jumps = 0 
        curr_idx = 0 
